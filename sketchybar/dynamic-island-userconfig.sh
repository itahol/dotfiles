#### DYNAMIC ISLAND USERCONFIG - MACBOOK PRO 2021 - 14 INCH #####
...
BUILTIN_MONITOR_ID=$(($CHOSEN_MONITOR_INDEX + 1))
P_DYNAMIC_ISLAND_DISPLAY=$BUILTIN_MONITOR_ID
...
case "$CHOSEN_DISPLAY_NAME" in
  "$BUILTIN_MONITOR_NAME")
    ...
    ;;
  *)
    # Make notch invisible for external displays
    P_DYNAMIC_ISLAND_DEFAULT_HEIGHT=0
    P_DYNAMIC_ISLAND_DEFAULT_WIDTH=0
    P_DYNAMIC_ISLAND_CORNER_RADIUS=10
    ;;
esac

# in case of built-in display, it must be divded by 2, e.g.: MBP 14 has 3024 resolution, so 3024/2=1512
P_DYNAMIC_ISLAND_MONITOR_HORIZONTAL_RESOLUTION=$CHOSEN_MONITOR_HORIZONTAL_RESOLUTION

# Animation Settings
P_DYNAMIC_ISLAND_SQUISH_AMOUNT=3
